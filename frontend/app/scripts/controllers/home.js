...
angular.module('wongoApp')
    .controller('HomeCtrl', function($scope, $location, $http, $log, $q, SchemaParser, SchemaEditorView, DataStore, MongodbUtils) {

        function retrieveDataByCollection(connectionUrl, collectionName) {
            var def = $q.defer();

            $http
                .get('http://localhost:3000/api/' + connectionUrl + '/' + collectionName)
        ...
        $scope.currentCollection = search.collectionName;

        // bind
        $scope.SchemaEditorView = SchemaEditorView;

        // set database name
        $scope.connectionUrl = DataStore.getCurrentConnectionUrl() || 'localhost:27017/test';
        $scope.currentDatabase = MongodbUtils.parseMongodbConnectionUrl($scope.connectionUrl).databaseName;



        if ($scope.currentCollection && $scope.currentCollection !== '') {
            retrieveDataByCollection($scope.connectionUrl, $scope.currentCollection).then(function(data) {
                $scope.data = data;
        ...
        // $log.info('$scope.collections', $scope.collections);
        DataStore.getDatabaseCollectionsByConnectionUrl(DataStore.getCurrentConnectionUrl()).then(function(collections) {
            if (!collections || collections.length === 0) {
                $scope.isEmptyDatabase = true;
                return;
            }
            $scope.isEmptyDatabase = false;
            $scope.currentCollection = collections[0];
            $log.info('$scope.currentCollection', $scope.currentCollection);
